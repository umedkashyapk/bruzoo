(function ($) {
  ...
  table = $("#table").DataTable({
    ...
    order: [],
    ajax: {
      url: BASE_URL + "admin/advertisment/list",
      type: "POST",
    },
  ...
  $(".is_goole_adsense").on("change", function (e) 
  {
    
    if($(this).prop("checked") == true) 
    {
      $('.url_or_image_div').addClass('d-none');
      $('.google_ad_code_div').removeClass('d-none');
    } 
    else 
    {
      $('.google_ad_code_div').addClass('d-none');
      $('.url_or_image_div').removeClass('d-none');
    }
