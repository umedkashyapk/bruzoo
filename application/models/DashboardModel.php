...
class DashboardModel extends CI_Model {
    ...
    function quiz_count() {
        return $this->db->select('count(*) as count')->where('is_quiz_active',1)->get('quizes')->row();
    }
    function study_material_count() {
        return $this->db->select('count(*) as count')->where('status',1)->get('study_material')->row();
    }
