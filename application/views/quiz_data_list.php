...
if($quiz_list_data)  
{
    ...
    foreach ($quiz_list_data as  $quiz_array) 
    {
        ...
        {
            $user_category_membership = get_user_category_membership($user_id,$quiz_array->category_id);
        ...
        if($quiz_array->is_sheduled_test == 1 && $quiz_array->start_date_time != 0 && $quiz_array->end_date_time != 33168837600)
        {
            ...
            if($quiz_array->start_date_time > $current_date_time)
            {
                $left_time = $quiz_array->start_date_time - $current_date_time;
        ...
        $box_to_show_on_md_row = isset($box_to_show_on_row) ? $box_to_show_on_row : 6;
        $box_to_show_on_lg_row = isset($box_to_show_on_row) ? $box_to_show_on_row+1 : 6;

        ?>
        <div class="col-xs-6 col-sm-6 col-md-<?php echo $box_to_show_on_md_row; ?> col-lg-<?php echo $box_to_show_on_lg_row; ?>"> 
            <!-- --------------------------------------------- -->
            <div class="card-section">
                    <div class="test-area-card card-body">
                    <button class="freepaid <?php echo $ribbon_label;?>">  <?php echo $quiz_price_ribbon ;?></button>
                    <?php
                $quiz_slug_url = base_url('quiz/').slugify_string($quiz_title)."-$quiz_id";
             ?>
                    <div class="exam-name"><a href="<?php echo xss_clean($quiz_slug_url); ?>" class="text-dark"><?php echo xss_clean($quiz_title); ?>
                        
                    </a></div>
                    <div class="span-text">
                        <span><i class="fa fa-question-circle" aria-hidden="true"></i> <?php echo xss_clean($quiz_array->number_questions); ?> <?php echo lang('questions') ?></span> 

                        <span><i class="fa fa-file" aria-hidden="true"></i> 15 Marks</span>

                        <span> <i class="fas fa-stopwatch" aria-hidden="true"></i> <?php echo xss_clean($quiz_array->duration_min);?> <?php echo lang('minutes') ?></span>
                    </div>
                 <div class="strat-btn">
                    <a href="<?php echo xss_clean($quiz_url); ?>" data-url="<?php echo xss_clean($quiz_url); ?>" id="quiz_<?php echo xss_clean($quiz_array->id);?>" class=" <?php echo xss_clean($quiz_running_btn); ?> <?php echo $start_quiz_btn_disable; ?> statrt_quiz_btn btn btn-sm w-100 <?php echo $quiz_action_btn_class; ?>" data-quiz_id="<?php echo xss_clean($quiz_array->id);?>" data-toggle="tooltip"  title="<?php echo $quiz_btn_name;?>"> <?php echo $quiz_action_icon." ".$quiz_btn_name;?>
                    
                </a>
                </div>
                </div>
                <div class="language"><i class="fa fa-language" aria-hidden="true"></i> English</div>
                </div>
...
<!--             <div class="card card-bundle mb-2" data-turbolinks="false"> 
                <div class="thumbnail"> 
                    ...
                    <a href="<?php echo xss_clean($quiz_url); ?>" data-url="<?php echo xss_clean($quiz_url); ?>" id="quiz_<?php echo xss_clean($quiz_array->id);?>" class="thumb-cover  <?php echo xss_clean($quiz_running_btn); ?> <?php echo $start_quiz_btn_disable ;?> statrt_quiz_btn" data-quiz_id="<?php echo xss_clean($quiz_array->id);?>" data-toggle="tooltip"  title="<?php  echo $quiz_btn_name;?>"> &nbsp; </a>
                    <div class="details card-dark-shadow">
                        ...
                        <div class="actions"> 
                            <a href="<?php echo xss_clean($quiz_url); ?>" data-url="<?php echo xss_clean($quiz_url); ?>" id="quiz_<?php echo xss_clean($quiz_array->id);?>" class="btn btn-neutral btn-fill <?php echo xss_clean($quiz_running_btn); ?> <?php echo $start_quiz_btn_disable ;?> statrt_quiz_btn" data-quiz_id="<?php echo xss_clean($quiz_array->id);?>" data-toggle="tooltip"  title="<?php  echo $quiz_btn_name;?>">
            ...
            <?php if($left_time > 0) { ?>
                <div class="my-0 infobox text-center">
                    <span class="upcoming <?php echo $quiz_timer; ?>" data-seconds-left="<?php echo $left_time; ?>"></span>            
                </div> 
            <?php } ?>    
        </div>
