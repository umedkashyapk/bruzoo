...
<div class="col-12 col-md-12 col-lg-12">
   <div class="card">
      <div class="card-body">
          ...
          <div class="col-3">
            <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
              <a class="nav-link active" id="v-pills-general-tab" data-toggle="pill" href="#v-pills-general" role="tab" aria-controls="v-pills-general" aria-selected="true">
                <?php echo lang('general_setting');?>
              </a>
              <a class="nav-link" id="v-pills-social-tab" data-toggle="pill" href="#v-pills-social" role="tab" aria-controls="v-pills-social" aria-selected="false">
                <?php echo lang('social_setting');?>
              </a>
              <a class="nav-link" id="v-pills-mail-tab" data-toggle="pill" href="#v-pills-mail" role="tab" aria-controls="v-pills-mail" aria-selected="false">
                <?php echo lang('mail_setting');?>
              </a>
              <a class="nav-link" id="v-pills-social-login-tab" data-toggle="pill" href="#v-pills-social-login" role="tab" aria-controls="v-pills-social-login" aria-selected="false">
         ...
         <?php 
         $general_setting = $social_setting = $mail_setting = $invoice_setting =  $payment_setting = $social_login_setting = $testimonial_setting = $header_setting = $all_setting = array();
        foreach ($settings as $setting) 
        {
          if ($setting['setting_group'] == 'general setting') 
          {
             $general_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'social setting') 
          {
             $social_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'mail setting') 
          {
             $mail_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'invoice setting') 
          {
             $invoice_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'payment setting') 
          {
             $payment_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'social login setting') 
          {
             $social_login_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'testimonial setting') 
          {
             $testimonial_setting[] = $setting;
             
          }
          if ($setting['setting_group'] == 'header setting') 
          {
         ...
         <?php // no translations
            ...
            $field_data['class'] = "form-control" . (($setting['show_editor']) ? " editor" : "");
            $field_data['value'] = isset($_POST[$setting['name']]) ? $_POST[$setting['name']] : $setting['value'];
            $field_data['group'] = $setting['setting_group'];
        ?>
         <div class="row">
            <div class="<?php echo xss_clean($col_size); ?>">
               <div class="form-group <?php echo form_error($setting['name']) ? ' has-error' : ''; ?>">
