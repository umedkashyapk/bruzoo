...
<div class="container"> 
  <div class="row"> 
    <div class="col-md-12">
      ...
      <?php  if($this->session->userdata('logged_in') && $this->session->userdata('logged_in')['role'] == 'admin' && current_url() == base_url('admin/report/summary/'.$participant_id)) 
        { 
          $encrypted_id = encrypt_decrypt('encrypt',$participant_id);

        ?>
        
        <a data-toggle="modal" data-target="#Modal_result" class="btn btn-warning btn float-right mr-3 text-white" /><?php echo lang('send_email'); ?></a>
        <a href="<?php echo base_url('admin/report/card/'.$encrypted_id); ?>" target="_blank" class="btn btn-danger btn float-right mr-3 text-white"><?php echo lang('download_pdf'); ?></a>
      <?php } ?>  
    </div>
...
<!--       <h2 class="heading">
        <?php
          ...
          echo ucfirst($quiz_title); ?>
          <?php echo lang('quiz_summary'); ?>
 ...
 <?php if($quiz_data->is_disable_result == 0) { ?> 
  <div class="row">
...
<div class="modal fade bd-example-modal-lg" id="Modal_result" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">      
      ...
      <div class="modal-body">
        <div class="result-modal">
         ...
         <?php echo form_open_multipart('', array('role'=>'form','novalidate'=>'novalidate','id'=>'result_file_mail')); ?>
          <input type="hidden" name="participant_id" class="participant-id" value="<?php echo $participant_id; ?>">
            <div class="col-12">
              <div class="form-group">
                <?php echo  form_label(lang('email'), 'coupon_for'); ?>
                <input type="text" name="email" id="email_field" class="form-control border" value="">
              </div>
            </div>
            <div class="col-12">
              <?php echo  form_label(lang('subject'), 'coupon_for'); ?>
              <input type="text" name="subject" id="subject_field" class="form-control border" value="">
            </div>
            <div class="col-12 mb-3">
              <?php echo  form_label(lang('message'), 'coupon_for'); ?>
              <textarea name="message" class="form-control border"></textarea> 
            </div>
            <div class="col-12">
              <?php echo  form_label(lang('send_type'), 'coupon_for'); ?><br/>
  ...
  <div class="row" id="print-areas">  
    ...
    <div class="col-md-5">
      ...
      {
        ?>
        <div class=" text-center w-100 my-5 scholer-card ">
         ...
         <img src="<?php echo base_url('/assets/images/card.jpg')?>" id = "animated-example" class = "animated fadeInLeft" >
        ...
        <table class="table table-bordered">
          <tbody>
            ...
            <tr class="answered">
              ...
              ?>
              <td><span class="py-1 px-3 mr-2" style="background-color:<?php echo $color_code; ?>"></span><?php echo lang('total_attem_ques'); ?> </td>
            ...
            ?>
            <tr class="notanswer">
              <td><span class="py-1 px-3 mr-2" style="background-color:<?php echo $color_code; ?>"></span><?php echo lang('not_attempted'); ?></td>
              <td>
                <?php 
            ...
            <tr class="marks_for_correct_answer">
              ...
              ?>
              <td><span class="py-1 px-3 mr-2" style="background-color:<?php echo $color_code; ?>"></span><?php echo lang('marks_for_correct_answer'); ?></td>
    ...
    <div class="clearfix"></div>
      ...
      <div class="col-md-12 my-5">
        <div class="table100 py-0 ver1  m-b-110 mobile-table" style="overflow-x: scroll;">
          ...
          <table>
            <thead>
              <tr class="row100 head "> 
                ...
                <?php if(isset($quiz_data->hide_correct_answer) && $quiz_data->hide_correct_answer == 0) { ?>
            ...
            <tbody>
              ...
              foreach ($user_question_data as  $question_data) 
              {
                ...
                <?php
                    //$question_title = $question_data['render_content'] == 0 ? xss_clean($question_data['question']) : htmlspecialchars(xss_clean($question_data['question']));
                ?>
                  
                <?php     
                    $question_title = $question_data['question'];
                    $render_content = (isset($question_data['render_content']) && $question_data['render_content']) ? 1 : 0;
                   
                    $question_title = $render_content == 0 ?  htmlspecialchars_decode($question_title) : htmlspecialchars(xss_clean($question_title));

                    
                ?>
                  <td class="cell100 result-column1 py-5"><?php echo sprintf("%02d", $l); ?>. 
                    ...
                    <td class="cell100 result-column3"> 
                      ...
                      else
                      {
                        ...
                        if($given_correct_choice)
                        {
                          ...
                          $correct_choice_string = $question_data['render_content'] == 0 ? xss_clean($correct_choice_string) : htmlspecialchars(xss_clean($correct_choice_string));
                          ?>
                            <li><?php echo $correct_choice_string; ?></li>
                          <?php 
                        }
                        else
                        {
...
<?php } else { ?>
  <h1><?php echo lang('thank_you_for_given_test_i_will_publish_result_soon'); ?></h1>
<?php  } ?>  
  
  <?php 
  
  if(empty($quiz_data->hide_correct_answer) OR $quiz_data->hide_correct_answer == 0)
  {
    foreach ($user_question_data as  $question_data) 
    { 
              ...
              <div class="left_content">
                <ul>
                  <!-- <strong> <?php echo lang('question'); ?> </strong> -->
                    <?php 
                      if(isset($question_data['question']) && !empty($question_data['question'])) 
                      { 
    
                        $translate_question_title = $question_data['question'];
                        $render_content = (isset($question_data['render_content']) && $question_data['render_content']) ? 1 : 0;
                        $translate_question_title2 = $render_content == 0 ? strip_tags($translate_question_title) : htmlspecialchars(xss_clean($translate_question_title));
                          
                        ?>
                        <div class="question-area">
                           <h5><?php echo $translate_question_title2; ?></h5>
                        </div>
                        <?php 
                      } ?>
                  <?php 
                  ...
                  <ul class="py-4">
                    <?php 
                      ...
                      {
                        $choices_val = $question_data['render_content'] == 0 ? xss_clean($choices_val) : htmlspecialchars(xss_clean($choices_val));
                    ?>
                      <li>                  
                        <?php
